...
# Ce script insère les données de référence nécessaires
...
# Base LCA - Facteurs d'ingrédients
echo "📊 Initialisation de la base LCA..."
PGPASSWORD=admin psql -h localhost -p 5435 -U postgres -d eco_lca <<EOF
...
('SODIUM_CHLORIDE', 0.5, 2.0, 1.0),
('CITRIC_ACID', 1.2, 8.0, 4.0),
('PARFUM', 3.0, 20.0, 10.0)
...
PGPASSWORD=admin psql -h localhost -p 5434 -U postgres -d ecolabel_nlp <<EOF
...
('Sodium Chloride', 'salt;sel', 'SODIUM_CHLORIDE'),
('Citric Acid', 'acide citrique', 'CITRIC_ACID'),
...
('Verre', 'glass', 'GLASS_GENERIC'),
('Carton', 'cardboard;papier;paper', 'CARDBOARD_GENERIC')
ON CONFLICT (name) DO NOTHING;

-- Taxonomie de labels
INSERT INTO label_taxonomy (name, synonyms) VALUES
('Vegan', 'vegane'),
('Bio', 'organic;biologique'),
('Recyclable', 'recyclable;100% recyclable'),
('Commerce équitable', 'fairtrade;fair trade')
ON CONFLICT (name) DO NOTHING;
EOF
